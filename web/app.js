...
(() => {
  ...
  const marketExpanded = {news:false, calendar:false, experts:false, portfolios:false};
  ...
  function renderMarket() {
    ...
    const macro = (Array.isArray(state.market.macroEvents) ? state.market.macroEvents : []).filter(e=>!e.date || String(e.date) >= today);
    ...
    const all = [...news].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0));
    const macroAll = [...macro].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||'')));
    const viewsAll = [...expertViews].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0));
    ...
    const moreButton = (count,key,limit=5) => count>limit ? `<button class="market-more" data-action="toggle-market" data-market-key="${key}">${marketExpanded[key]?'Daha az göster':'Tümünü göster'} <span>${marketExpanded[key]?'↑':'↓'}</span></button>` : '';
    const newsCards = limitItems(all,'news').map(n=>clickable(n.url,'content-news',`<div class="content-kicker">${esc(n.publisher||n.source||'Haber')} · ${n.publishedAt?timeAgo(n.publishedAt):''}</div><div class="content-title">${esc(n.title)}</div><div class="content-summary">${esc(n.summary||'Ayrıntılar için habere dokunun.')}</div>`)).join('');
    const macroCards = limitItems(macroAll,'calendar').map(e=>clickable(e.url,'content-calendar',`<div class="content-kicker">${esc(e.source||'Resmi takvim')} · ${esc(e.date||'')}</div><div class="content-title">${esc(e.title)}</div>${e.period?`<div class="content-summary">${esc(e.period)}</div>`:''}`)).join('');
    const viewCards = limitItems(viewsAll,'experts').map(v=>clickable(v.url,'content-expert',`<div class="content-kicker">${esc(v.publisher||v.source||'Uzman görüşü')} · ${v.publishedAt?timeAgo(v.publishedAt):''}</div><div class="content-title">${esc(v.title)}</div><div class="content-summary">${esc(v.summary||'Görüşün ayrıntıları için dokunun.')}</div>`)).join('');
    const portfolioRows = investors.map(r=>{
      ...
      const summary = top.length ? top.map(h=>{const pct=total?Number(h.value||0)/total*100:0;return `${esc(h.name||'Varlık')} %${numberFmt(pct,1)}`}).join(' · ') : 'Son 13F bildirimi şu an alınamadı. SEC kaynak sayfasını açabilirsiniz.';
    ...
    const portfolioCards = limitItems(portfolioRows,'portfolios',4).join('');
    return `${pageHeader('Piyasa','Haberler ve gelişmeler','Kısa özetler, yaklaşan ekonomik takvim ve uzman/portföy takibi',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      ${backendNotice}
      <section class="card market-card market-news"><div class="section-head"><div><div class="section-title">Önemli haberler</div><div class="section-note">Kısa özet için kartı oku; ayrıntı için karta dokun.</div></div></div><div class="content-list">${newsCards || emptyState('news','Henüz haber yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div>${moreButton(all.length,'news')}</section>
      <section class="card market-card market-calendar"><div class="section-head"><div><div class="section-title">Yaklaşan Türkiye ekonomik takvimi</div><div class="section-note">Bugünden sonraki TÜİK ve TCMB resmi yayınları.</div></div></div><div class="content-list">${macroCards || emptyState('calendar','Yaklaşan takvim verisi yok','Yenile düğmesine basın.')}</div>${moreButton(macroAll.length,'calendar')}</section>
      <section class="card market-card market-experts"><div class="section-head"><div><div class="section-title">Uzman görüşleri</div><div class="section-note">Kamuya açık güncel yorumlar; yatırım tavsiyesi değildir.</div></div></div><div class="content-list">${viewCards || emptyState('people','Henüz uzman görüşü yok','Yenile düğmesine basın.')}</div>${moreButton(viewsAll.length,'experts')}</section>
      <section class="card market-card market-portfolios"><div class="section-head"><div><div class="section-title">Kamuya açık yatırımcı portföyleri</div><div class="section-note">SEC 13F bildirimlerinden; gecikmeli ve yatırım tavsiyesi değildir.</div></div></div><div class="content-list">${portfolioCards || emptyState('portfolio','Henüz portföy verisi yok','Yenile düğmesine basın.')}</div>${moreButton(portfolioRows.length,'portfolios',4)}</section>`;
  }
  ...
  function handleAction(action, node) {
    const actions = {
      ...
      'toggle-market': () => { const key=node?.dataset.marketKey; if(key && Object.prototype.hasOwnProperty.call(marketExpanded,key)){ marketExpanded[key]=!marketExpanded[key]; renderPage(); } }
    };
    actions[action]?.();
  }
