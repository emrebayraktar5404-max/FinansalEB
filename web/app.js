...
(() => {
  ...
  function assetCost(asset) { return Number(asset.quantity || 0) * Number(asset.avgCost || 0) * fxRate(asset.currency); }
  ...
  function quantityAtDate(assetId, dateValue) {
    const asset = assetById(assetId);
    if (!asset) return 0;
    const cutoff = parseDate(dateValue || isoDate());
    const txs = state.transactions
      .filter(t => t.assetId === assetId && (t.type === 'buy' || t.type === 'sell') && t.date)
      .sort((a,b) => parseDate(a.date) - parseDate(b.date));
    if (!txs.length) return Number(asset.quantity || 0);
    return Math.max(0, txs.reduce((qty,t) => {
      if (parseDate(t.date) > cutoff) return qty;
      return qty + (t.type === 'buy' ? Number(t.quantity||0) : -Number(t.quantity||0));
    }, 0));
  }
  function eventGross(event) {
    const asset = assetById(event.assetId);
    if (!asset) return 0;
    const eligibleQty = quantityAtDate(event.assetId, event.exDate || event.payDate);
  ...
  function renderMarket() {
    const news = Array.isArray(state.market.news) ? state.market.news : [];
    const macro = Array.isArray(state.market.macroEvents) ? state.market.macroEvents : [];
    const expertViews = Array.isArray(state.market.expertViews) ? state.market.expertViews : [];
    const investors = Array.isArray(state.market.investorPortfolios) ? state.market.investorPortfolios : [];
    const all = [...news].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)).slice(0,10);
    const macroTop = [...macro].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))).slice(0,10);
    const viewsTop = [...expertViews].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)).slice(0,8);
    const backendNotice = !state.settings.backendUrl ? `<div class="market-notice"><strong>Canlı içerik sunucusu tanımlı değil.</strong><span>Haber, resmi takvim ve kamuya açık portföyler için Ayarlar → Veri sunucusu bölümüne kendi api.php adresinizi ekleyin.</span></div>` : '';
    const clickable = (href, cls, body) => `<a class="content-item ${cls}" href="${esc(href||'#')}" target="_blank" rel="noopener">${body}<span class="content-open">Aç →</span></a>`;
    const newsCards = all.map(n=>clickable(n.url,'content-news',`<div class="content-kicker">${esc(n.publisher||n.source||'Haber')} · ${n.publishedAt?timeAgo(n.publishedAt):''}</div><div class="content-title">${esc(n.title)}</div><div class="content-summary">${esc(n.summary||'Ayrıntılar için habere dokunun.')}</div>`)).join('');
    const macroCards = macroTop.map(e=>clickable(e.url,'content-calendar',`<div class="content-kicker">${esc(e.source||'Resmi takvim')} · ${esc(e.date||'')}</div><div class="content-title">${esc(e.title)}</div>${e.period?`<div class="content-summary">${esc(e.period)}</div>`:''}`)).join('');
    const viewCards = viewsTop.map(v=>clickable(v.url,'content-expert',`<div class="content-kicker">${esc(v.publisher||v.source||'Uzman görüşü')} · ${v.publishedAt?timeAgo(v.publishedAt):''}</div><div class="content-title">${esc(v.title)}</div><div class="content-summary">${esc(v.summary||'Görüşün ayrıntıları için dokunun.')}</div>`)).join('');
    const portfolioCards = investors.map(r=>{
      const holdings = Array.isArray(r.holdings)?r.holdings:[];
      const total = holdings.reduce((sum,h)=>sum+Number(h.value||0),0);
      const top = holdings.slice().sort((a,b)=>Number(b.value||0)-Number(a.value||0)).slice(0,5);
      const summary = top.length ? top.map(h=>{const pct=total?Number(h.value||0)/total*100:0;return `${esc(h.name||h.ticker||'Varlık')} %${numberFmt(pct,1)}`}).join(' · ') : 'SEC 13F verisi geçici olarak alınamadı. Kaynak sayfasını açabilirsiniz.';
      return clickable(r.url,'content-portfolio',`<div class="content-kicker">${esc(r.source||'SEC 13F')}${r.filingDate?` · ${esc(r.filingDate)}`:''}</div><div class="content-title">${esc(r.manager)}</div><div class="content-summary">${summary}</div>`);
    }).join('');
    return `${pageHeader('Piyasa','Haberler ve gelişmeler','Kısa özetler, ekonomik takvim ve uzman/portföy takibi',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      ${backendNotice}
      <section class="card market-card market-news"><div class="section-head"><div><div class="section-title">Önemli haberler</div><div class="section-note">Kısa özet için kartı oku; ayrıntı için karta dokun.</div></div></div><div class="content-list">${newsCards || emptyState('news','Henüz haber yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div></section>
      <section class="card market-card market-calendar"><div class="section-head"><div><div class="section-title">Türkiye ekonomik takvimi</div><div class="section-note">TÜİK ve TCMB resmi yayınları.</div></div></div><div class="content-list">${macroCards || emptyState('calendar','Takvim verisi yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div></section>
      <section class="card market-card market-experts"><div class="section-head"><div><div class="section-title">Uzman görüşleri</div><div class="section-note">Kamuya açık güncel yorumlar; yatırım tavsiyesi değildir.</div></div></div><div class="content-list">${viewCards || emptyState('people','Henüz uzman görüşü yok','Yenile düğmesine basın.')}</div></section>
      <section class="card market-card market-portfolios"><div class="section-head"><div><div class="section-title">Örnek / kamuya açık portföyler</div><div class="section-note">SEC 13F bildirimlerinden; gecikmeli ve yatırım tavsiyesi değildir.</div></div></div><div class="content-list">${portfolioCards || emptyState('portfolio','Henüz portföy verisi yok','Yenile düğmesine basın.')}</div></section>`;
  }
  ...
  async function refreshContent() {
    try {
      const result = await backendCall({action:'content'});
    ...
    form.addEventListener('submit',async e=>{
      ...
      try {
        ...
        if (existing) Object.assign(existing,next); else {
          ...
          if (next.quantity > 0) state.transactions.push({id:uid('tx'),assetId:next.id,type:'buy',date:String(fd.get('purchaseDate')||isoDate()),quantity:next.quantity,price:next.avgCost,fee:0,currency:next.currency});
        }
        state.demo=false;
        saveState();
        closeModal();
  ...
  function showTransactionForm(assetId = null) {
    ...
    $('#txForm').addEventListener('submit',e=>{
      ...
      const type=fd.get('type'), qty=Number(fd.get('quantity')||0), price=Number(fd.get('price')||0), fee=Number(fd.get('fee')||0);
      if(type==='dividend') { state.dividendEvents.push({id:uid('div'),assetId:a.id,exDate:String(fd.get('date')),payDate:String(fd.get('date')),amountPerShare:qty?price/qty:price,currency:a.currency,status:'confirmed',received:true,source:'Manuel işlem'}); }
        ...
        const oldQty=Number(a.quantity||0); if(type==='buy'){const newQty=oldQty+qty;a.avgCost=newQty?((oldQty*Number(a.avgCost||0))+(qty*price)+fee)/newQty:0;a.quantity=newQty;} else a.quantity=Math.max(0,oldQty-qty);
        state.transactions.push({id:uid('tx'),assetId:a.id,type,date:String(fd.get('date')),quantity:qty,price,fee,currency:a.currency});
      }
  ...
  async function backendCall(params) {
