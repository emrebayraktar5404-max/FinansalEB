...
(() => {
  ...
  function eventGross(event) {
    ...
    return asset ? Number(asset.quantity || 0) * Number(event.amountPerShare || 0) : 0;
  }
  ...
  function eventRow(event) {
    ...
    return `<article class="event-row" data-asset-id="${esc(asset.id)}">
      ...
      <div class="event-main"><div class="event-title">${esc(asset.symbol)} · ${event.payDate ? 'Temettü ödemesi' : 'Hak kullanım'}</div><div class="event-meta">${meta} · ${numberFmt(asset.quantity, asset.type==='TEFAS'?0:2)} adet</div><span class="status-pill ${eventStatusMeta(event).className}">${eventStatusMeta(event).label}</span></div>
  ...
  function renderMarket() {
    ...
    const all = [...news].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)).slice(0,20);
    const macroTop = [...macro].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))).slice(0,20);
    return `${pageHeader('Piyasa','Haberler ve önemli gelişmeler','KAP, resmi makro takvim ve piyasa haberleri',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      <section class="card"><div class="section-head"><div><div class="section-title">Önemli haberler</div><div class="section-note">Kaynak ve yayın zamanı gösterilir.</div></div></div>
      <div class="content-list">${all.length ? all.map(n=>`<article class="content-item"><div class="content-kicker">${esc(n.source||'Haber')} · ${n.publishedAt?timeAgo(n.publishedAt):''}</div><div class="content-title">${esc(n.title)}</div><div class="content-summary">${esc(n.summary||'')}</div><a class="content-link" href="${esc(n.url||'#')}" target="_blank" rel="noopener">Kaynağı aç →</a></article>`).join('') : emptyState('news','Henüz haber yok','Yenile düğmesine basın veya veri sunucusunu tanımlayın.')}</div></section>
      <section class="card"><div class="section-head"><div><div class="section-title">Türkiye ekonomik takvimi</div><div class="section-note">TÜİK ve TCMB resmi yayınları.</div></div></div>
      <div class="content-list">${macroTop.length ? macroTop.map(e=>`<article class="content-item"><div class="content-kicker">${esc(e.source||'Resmi takvim')} · ${esc(e.time||'')}</div><div class="content-title">${esc(e.title)}</div><div class="content-summary">${esc(e.period||'')}</div><a class="content-link" href="${esc(e.url||'#')}" target="_blank" rel="noopener">Resmi kaynağı aç →</a></article>`).join('') : emptyState('calendar','Takvim verisi yok','TÜİK/TCMB takvimi henüz alınamadı.')}</div></section>`;
  }

  function renderInvestors() {
    const rows = Array.isArray(state.market.investorPortfolios) ? state.market.investorPortfolios : [];
    return `${pageHeader('Yatırımcılar','Örnek portföyler','Kamuya açık kurumsal bildirimlerden derlenen örnekler',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      <section class="card"><div class="section-head"><div><div class="section-title">Kamuya açık portföyler</div><div class="section-note">Bunlar yatırım tavsiyesi değildir; bildirim tarihindeki pozisyonlardır.</div></div></div>
      <div class="content-list">${rows.length ? rows.map(r=>`<article class="content-item"><div class="content-kicker">${esc(r.source||'SEC 13F')} · ${esc(r.filingDate||'')}</div><div class="content-title">${esc(r.manager)}</div><div class="content-summary">${(r.holdings||[]).slice(0,8).map(h=>`${esc(h.ticker||h.name)} · ${numberFmt(h.value||0,0)}$`).join(' · ')}</div><a class="content-link" href="${esc(r.url||'#')}" target="_blank" rel="noopener">Orijinal bildirimi aç →</a></article>`).join('') : emptyState('people','Örnek portföy bulunamadı','SEC verisi için veri sunucusunu kullanın.')}</div></section>`;
  }
