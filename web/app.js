  ...
  function showTransactionForm(assetId = null) {
    ...
    $('#txForm').addEventListener('submit',e=>{
      ...
      if(type==='dividend') { const evt={id:uid('div'),assetId:a.id,exDate:String(fd.get('date')),payDate:String(fd.get('date')),amountPerShare:qty?price/qty:price,currency:a.currency,status:'confirmed',received:true,source:'Manuel işlem'}; state.dividendEvents.push(evt); state.cashLedger.push({id:uid('cash'),type:'dividend_income',eventId:evt.id,date:evt.payDate,amountTry:eventNet(evt),note:`${a.symbol} net temettü`}); }
        ...
        state.transactions.push({id:uid('tx'),assetId:a.id,type,date:txDate,quantity:qty,price,fee,currency:a.currency,fundingSource:type==='buy'?fundingSource:undefined});
        syncAssetLedger(a);
      }
