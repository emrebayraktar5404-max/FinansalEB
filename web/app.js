...
(() => {
  ...
  function renderMarket() {
    ...
    const all = [...news].sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)).slice(0,12);
    const macroTop = [...macro].sort((a,b)=>String(a.date||'').localeCompare(String(b.date||''))).slice(0,12);
    ...
    return `${pageHeader('Piyasa','Haberler ve gelişmeler','Haber, ekonomik takvim ve uzman/portföy takibi',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      ${backendNotice}
      <section class="card market-card"><div class="section-head"><div><div class="section-title">Önemli haberler</div><div class="section-note">Kaynak ve yayın zamanı gösterilir.</div></div></div>
      <div class="content-list">${all.length ? all.map(n=>`<article class="content-item"><div class="content-kicker">${esc(n.source||'Haber')} · ${n.publishedAt?timeAgo(n.publishedAt):''}</div><div class="content-title">${esc(n.title)}</div><div class="content-summary">${esc(n.summary||'')}</div><a class="content-link" href="${esc(n.url||'#')}" target="_blank" rel="noopener">Kaynağı aç →</a></article>`).join('') : emptyState('news','Henüz haber yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div></section>
      <section class="card market-card"><div class="section-head"><div><div class="section-title">Türkiye ekonomik takvimi</div><div class="section-note">TÜİK ve TCMB resmi yayınları.</div></div></div>
      <div class="content-list">${macroTop.length ? macroTop.map(e=>`<article class="content-item"><div class="content-kicker">${esc(e.source||'Resmi takvim')} · ${esc(e.time||'')}</div><div class="content-title">${esc(e.title)}</div><div class="content-summary">${esc(e.period||'')}</div><a class="content-link" href="${esc(e.url||'#')}" target="_blank" rel="noopener">Resmi kaynağı aç →</a></article>`).join('') : emptyState('calendar','Takvim verisi yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div></section>
      <section class="card market-card"><div class="section-head"><div><div class="section-title">Uzman görüşleri & örnek portföyler</div><div class="section-note">Kamuya açık kaynaklar; yatırım tavsiyesi değildir.</div></div></div>
      <div class="content-list">${investors.length ? investors.map(r=>`<article class="content-item"><div class="content-kicker">${esc(r.source||'Kamuya açık bildirim')} · ${esc(r.filingDate||'')}</div><div class="content-title">${esc(r.manager)}</div><div class="content-summary">${(r.holdings||[]).slice(0,6).map(h=>`${esc(h.ticker||h.name)} · ${numberFmt(h.value||0,0)}$`).join(' · ')}</div><a class="content-link" href="${esc(r.url||'#')}" target="_blank" rel="noopener">Kaynağı aç →</a></article>`).join('') : emptyState('people','Henüz görüş/portföy yok',state.settings.backendUrl?'Yenile düğmesine basın.':'Önce veri sunucusunu tanımlayın.')}</div></section>`;
  }
