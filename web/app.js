...
(() => {
  ...
  function portfolioMetrics() {
    ...
    const allAnnualFallback = state.assets.reduce((sum,a) => sum + Number(a.quantity||0) * Number(a.annualDividendPerShare||0) * (1-clamp(automaticDividendTax(a),0,100)/100) * fxRate(a.currency), 0);
  ...
  function showAssetForm(assetId = null) {
    ...
    showModal(`${modalHeader(existing ? 'Varlığı düzenle' : 'Yeni varlık ekle')}
      <form id="assetForm"><div class="form-grid">
        ...
        <div class="field fund-fee-field" style="${a.type==='TEFAS'?'':'display:none'}"><label>Fon yönetim ücreti (%/yıl)</label><input name="fundManagementFeeAnnual" type="number" step=".0001" min="0" value="${a.fundManagementFeeAnnual ?? ''}" placeholder="KAP'tan otomatik"></div>
        <div class="field fund-fee-field" style="${a.type==='TEFAS'?'':'display:none'}"><label>Fon toplam gider oranı (%)</label><input name="fundExpenseRatio" type="number" step=".0001" min="0" value="${a.fundExpenseRatio ?? ''}" placeholder="Varsa KAP'tan"></div>
        <div class="field fund-fee-field" style="${a.type==='TEFAS'?'':'display:none'}"><label>Giriş / çıkış komisyonu (%)</label><div style="display:flex;gap:8px"><input name="fundEntryFee" type="number" step=".0001" min="0" value="${a.fundEntryFee ?? ''}" placeholder="Giriş"><input name="fundExitFee" type="number" step=".0001" min="0" value="${a.fundExitFee ?? ''}" placeholder="Çıkış"></div></div>
        <div class="field fund-fee-field" style="${a.type==='TEFAS'?'':'display:none'}"><label>Performans ücreti (%)</label><input name="fundPerformanceFee" type="number" step=".0001" min="0" value="${a.fundPerformanceFee ?? ''}" placeholder="Varsa KAP'tan"></div>
    ...
    form.addEventListener('submit',async e=>{
      ...
      try {
        ...
        const next = {
          ...
          fundManagementFeeDaily:selectedMarketData?.fundFees?.managementFeeDaily ?? existing?.fundManagementFeeDaily ?? null,
