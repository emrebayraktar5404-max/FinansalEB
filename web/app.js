...
(() => {
  ...
  function ipoProgressHtml(group){
    const a=group.asset;
    const hist=(a.history||[]).map(Number).filter(Number.isFinite);
    const dates=Array.isArray(a.historyDates)?a.historyDates:[];
    if(!hist.length)return '<div class="empty-text" style="margin:0">Günlük fiyat geçmişi ilk piyasa yenilemesinden sonra oluşacak.</div>';
  ...
  function showTransactionForm(assetId = null, purchaseKindDefault = 'normal') {
    ...
    showModal(`${modalHeader('Yeni portföy işlemi')}<form id="txForm">
      ...
      <div class="form-grid"><div class="field full"><label>Varlık</label><select name="assetId">${state.assets.map(a=>`<option value="${a.id}" ${a.id===assetId?'selected':''}>${esc(a.symbol)} · ${esc(a.name)}</option>`).join('')}</select></div><div class="field"><label>Tarih</label><input type="date" name="date" value="${isoDate()}" required></div><div class="field"><label>Adet / pay</label><input type="number" step="any" min="0" name="quantity" required></div><div class="field"><label>Fiyat / pay</label><input type="number" step="any" min="0" name="price" required></div><div class="field"><label>Komisyon</label><input type="number" step="any" min="0" name="fee" value="0"></div><div class="field"><label>Alış kaynağı</label><select name="fundingSource"><option value="new_money">Yeni para / normal bakiye</option><option value="dividend">Temettü TL bakiyesi</option></select><div class="field-hint">Temettü bakiyesi: ${money(dividendCashBalance(),'TRY')}</div></div><div class="field full"><label>Alış türü</label><select name="purchaseKind"><option value="normal" ${purchaseKindDefault==='normal'?'selected':''}>Normal piyasa alımı</option><option value="ipo" ${purchaseKindDefault==='ipo'?'selected':''}>Halka arzdan dağıtılan lot</option></select><div class="field-hint">Halka arzı seçersen bu lotlar Piyasa → Halka Arzlar → Aldıklarım alanında ayrı izlenir.</div></div></div>
    ...
    $('#txForm').addEventListener('submit',e=>{
      ...
      if(type==='dividend') { const evt={id:uid('div'),assetId:a.id,exDate:String(fd.get('date')),payDate:String(fd.get('date')),amountPerShare:qty?price/qty:price,currency:a.currency,status:'confirmed',received:true,source:'Manuel işlem'}; state.dividendEvents.push(evt); state.cashLedger.push({id:uid('cash'),type:'dividend_income',eventId:evt.id,date:evt.payDate,amountTry:eventNet(evt),note:`${a.symbol} net temettü`}); }
      else {
        ...
        if(type==='buy'&&fundingSource==='dividend'){
          ...
          state.cashLedger.push({id:uid('cash'),type:'dividend_reinvestment',date:txDate,amountTry:-spendTry,note:`${a.symbol} temettü yeniden yatırımı`});
        }
