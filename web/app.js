...
(() => {
  ...
  function renderMarket() {
    ...
    const clickable = (href, cls, body) => `<a class="content-item ${cls}" href="${esc(href||'#')}" rel="noopener">${body}<span class="content-open">Aç →</span></a>`;
    ...
    const portfolioCards = limitItems(investors,'portfolios',4).map(r=>{
      ...
      return clickable(r.url,'content-portfolio',`<div class="content-kicker">🌎 ${esc(r.source||'Kamuya açık portföy')}${r.filingDate?` · ${esc(r.filingDate)}`:''}</div><div class="content-title">${esc(r.manager)}</div><div class="content-summary">${esc(summary)}</div>`);
    }).join('');

    const common=(comparison.commonStocks||[]).slice(0,12);
    const compareStockHtml = `<div class="compare-summary">
      <div class="compare-stat"><b>${domestic.filter(x=>(x.holdings||[]).length).length}</b><span>okunan Türk portföyü</span></div>
      <div class="compare-stat"><b>${common.length}</b><span>ortak hisse</span></div>
      <div class="compare-stat"><b>${funds.length}</b><span>fon kaynağı</span></div>
    </div>
    ...
    const compareFundHtml = `<div class="fund-compare-grid">${funds.map(f=>`<a href="${esc(f.url)}" rel="noopener" class="fund-provider-mini"><b>${esc(f.manager)}</b><span>${(f.funds||[]).map(x=>esc(x.code)).join(' · ')}</span></a>`).join('')}</div>
    ...
    const compareWorldHtml = `<div class="ticker-wrap compare-tickers">${investors.map(x=>`<a class="ticker-chip strong" href="${esc(x.url)}" rel="noopener">${esc(x.manager)}</a>`).join('')}</div><div class="compare-tip">ABD portföyleri 13F bildirimleri nedeniyle gecikmelidir. Günlük işlem sinyali olarak kullanılmamalıdır.</div>`;

    const sourceHtml=limitItems(sourceCatalog,'sources',8).map(s=>`<a class="source-mini" href="${esc(s.url)}" rel="noopener"><span><b>${esc(s.name)}</b><small>${esc(s.role||'Kaynak')}</small></span><em>↗</em></a>`).join('');
    ...
    return `${pageHeader('Piyasa','Sade piyasa','Bugün ne oldu, yaklaşan tarihler ve profesyoneller ne izliyor?',`<button class="header-action" data-action="refresh-content">Yenile</button>`)}
      ...
      <section class="card market-card market-compare"><div class="section-head"><div><div class="section-title">⚖️ Portföy karşılaştırma</div><div class="section-note">Kurumların hisse ve fon fikirlerini aynı yerde karşılaştır.</div></div></div>
        <div class="segment-tabs">
          <button class="${marketCompareMode==='stocks'?'active':''}" data-action="market-compare" data-compare-mode="stocks">Hisseler</button>
          <button class="${marketCompareMode==='funds'?'active':''}" data-action="market-compare" data-compare-mode="funds">Fonlar</button>
          <button class="${marketCompareMode==='world'?'active':''}" data-action="market-compare" data-compare-mode="world">Dünya</button>
        </div>
      ...
      <section class="card market-card market-portfolios"><div class="section-head"><div><div class="section-title">🌎 Ünlü yatırımcı portföyleri</div><div class="section-note">Takip ve fikir edinme amaçlı; bildirimler gecikmeli olabilir.</div></div></div><div class="content-list">${portfolioCards || emptyState('portfolio','Henüz portföy verisi yok','Yenile düğmesine basın.')}</div>${moreButton(investors.length,'portfolios',4)}</section>
  ...
  function bindPageEvents() {
    $$('a[href]').forEach(a=>a.addEventListener('click',e=>{
      ...
      if (window.Android?.openExternal) {
        e.preventDefault();
        try { window.Android.openExternal(href); } catch (_) {}
      }
    }));
  ...
  function handleAction(action, node) {
    const actions = {
      ...
      'market-compare': () => { const mode=node?.dataset.compareMode; if(['stocks','funds','world'].includes(mode)){ marketCompareMode=mode; renderPage(); } }
    };
    actions[action]?.();
  }
