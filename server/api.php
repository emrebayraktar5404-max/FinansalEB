...
{
    // Decode first, then strip tags: RSS descriptions often encode <a ...> as entities.
...
{
    $desc = trim($desc);
    // Google RSS often repeats the title and publisher; avoid noisy duplicate text.
    if ($desc === '' || mb_strlen($desc) > 360 || str_contains($desc, 'http://') || str_contains($desc, 'https://')) {
        return $publisher !== '' ? 'Kaynak: '.$publisher : 'Ayrıntılar için habere dokunun.';
    }
    $normalizedTitle = mb_strtolower(trim($title));
    $normalizedDesc = mb_strtolower(trim($desc));
    if ($normalizedDesc === $normalizedTitle || str_starts_with($normalizedDesc, $normalizedTitle)) {
        return $publisher !== '' ? 'Kaynak: '.$publisher : 'Ayrıntılar için habere dokunun.';
    }
    if (mb_strlen($desc) > 180) $desc = rtrim(mb_substr($desc,0,177)).'…';
    return $desc;
}
