...
try {
    switch ($action) {
        ...
        case 'content':
            $data = cached('content_v8', 900, fn() => fetchContentBundle($config));
            respond(['ok' => true, 'data' => $data]);
            break;
...
function fetchContentBundle(array $config): array
{
    ...
    // Yeniden tarihe göre sıralarken de bir yayıncı ilk ekranı tek başına doldurmasın.
    $balanced=[]; $publisherCounts=[];
    foreach($news as $item){
        $pub=(string)($item['publisher']??$item['source']??'');
        if(($publisherCounts[$pub]??0)>=6) continue;
        $publisherCounts[$pub]=($publisherCounts[$pub]??0)+1; $balanced[]=$item;
    }
    $news=$balanced;
...
function fetchDomesticModelPortfolios(array $config): array
{
    $sources = [
        ...
        ['manager'=>'İş Yatırım','url'=>'https://www.isyatirim.com.tr/tr-tr/analiz/Sayfalar/is-yatirimin-onerileri.aspx','typeLabel'=>'Hisse model portföyü','parser'=>'generic'],
        ['manager'=>'Garanti BBVA Yatırım','url'=>'https://www.garantibbvayatirim.com.tr/medium/researchreports-constant-59160-2x.vsf','typeLabel'=>'Hisse model portföyü','parser'=>'generic'],
        ['manager'=>'Yapı Kredi Yatırım','url'=>'https://www.ykyatirim.com.tr/hizmetler/arastirma/model-portfoy-hisse-senedi-onerileri','typeLabel'=>'Hisse model portföyü','parser'=>'generic'],
        ['manager'=>'Oyak Yatırım','url'=>'https://www.oyakyatirim.com.tr/ModelPortfoy/TR','typeLabel'=>'Hisse model portföyü','parser'=>'generic'],
        ['manager'=>'Ahlatcı Yatırım','url'=>'https://www.ahlatciyatirim.com.tr/arastirma/gunluk-model-portfoy','typeLabel'=>'Günlük model portföy kaynağı','parser'=>'generic'],
    ];
    $out=[];
    foreach($sources as $src){
        try{
            if($src['parser']==='gedik') $row=fetchGedikModelPortfolio($src['url'],$config);
            elseif($src['parser']==='deniz') $row=fetchDenizModelPortfolio($src['url'],$config);
            else $row=fetchGenericModelPortfolio($src['url'],$config);
