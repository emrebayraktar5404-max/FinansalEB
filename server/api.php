...
$config = is_file($configFile) ? require $configFile : [
    ...
    'USER_AGENT' => 'FinansalEB/0.3.4 (personal portfolio tracker)',
    'SEC_USER_AGENT' => '',
];
...
try {
    switch ($action) {
        ...
        case 'content':
            $data = cached('content_v6', 900, fn() => fetchContentBundle($config));
            respond(['ok' => true, 'data' => $data]);
            break;
...
function fetchContentBundle(array $config): array
{
    $news = [];
    $queries = [
        ['q' => 'Borsa İstanbul ekonomi piyasa Türkiye', 'source' => 'Piyasa haberleri'],
        ['q' => 'site:tr.investing.com Borsa İstanbul ekonomi', 'source' => 'Investing.com'],
        ['q' => 'site:kap.org.tr özel durum açıklaması BIST', 'source' => 'KAP'],
        ['q' => 'Türkiye faiz enflasyon TCMB TÜİK', 'source' => 'Makro ekonomi'],
    ];
    foreach ($queries as $q) {
        try { $news = array_merge($news, fetchGoogleNewsRss($q['q'], $q['source'], $config)); }
        catch (Throwable $e) { error_log('[content/news] '.$e->getMessage()); }
    }
    ...
    $expertViews = [];
    $expertQueries = [
        ['q'=>'Mahfi Eğilmez ekonomi piyasa yorum', 'source'=>'Mahfi Eğilmez'],
        ['q'=>'Özgür Demirtaş ekonomi piyasa yorum', 'source'=>'Özgür Demirtaş'],
        ...
        ['q'=>'Warren Buffett portfolio market outlook', 'source'=>'Warren Buffett'],
        ['q'=>'Ray Dalio market outlook portfolio', 'source'=>'Ray Dalio'],
        ['q'=>'Michael Burry portfolio market', 'source'=>'Michael Burry'],
    ];
...
function fetchDomesticModelPortfolios(array $config): array
{
    ...
    foreach($sources as $src){
        try{
            ...
            $out[]=['manager'=>$src['manager'],'url'=>$src['url'],'typeLabel'=>$src['typeLabel'],'holdings'=>[],'updatedAt'=>'','status'=>'temporarily_unavailable'];
        }
    }
    // Model portföy yayınlamayan veya yapısı düzenli okunamayan kurumlar kaynak olarak izlenir.
    $out[]=['manager'=>'İş Yatırım / İş Portföy','url'=>'https://www.isportfoy.com.tr/getiri-ve-fiyatlar','typeLabel'=>'Fon ve araştırma kaynağı','holdings'=>[],'updatedAt'=>'','status'=>'source_only'];
    $out[]=['manager'=>'Garanti BBVA Yatırım / Portföy','url'=>'https://www.garantibbvaportfoy.com.tr/','typeLabel'=>'Fon ve araştırma kaynağı','holdings'=>[],'updatedAt'=>'','status'=>'source_only'];
    $out[]=['manager'=>'Ak Portföy','url'=>'https://www.akportfoy.com.tr/tr/fon/yatirim-fonlari/getiri?type=compare','typeLabel'=>'Fon kaynağı','holdings'=>[],'updatedAt'=>'','status'=>'source_only'];
    $out[]=['manager'=>'Yapı Kredi Yatırım / Portföy','url'=>'https://www.ykportfoy.com.tr/','typeLabel'=>'Fon ve araştırma kaynağı','holdings'=>[],'updatedAt'=>'','status'=>'source_only'];
    $out[]=['manager'=>'Ahlatcı Yatırım / Portföy','url'=>'https://www.ahlatciportfoy.com.tr/fonlar','typeLabel'=>'Fon ve araştırma kaynağı','holdings'=>[],'updatedAt'=>'','status'=>'source_only'];
    return $out;
}
...
function fetchGoogleNewsRss(string $query, string $source, array $config): array
{
    ...
    foreach ($items[1] ?? [] as $item) {
        ...
        if (str_contains($summary, 'Ayrıntılar için karta dokunun.')) {
            try { $summary = fetchArticleMetaSummary($link, $summary, $config); } catch (Throwable $e) { /* RSS özetiyle devam */ }
        }
...
function fetchTcmbCalendar(array $config): array
{
    ...
    foreach($rows[1]??[] as $rowHtml){
        ...
        foreach($parts as $i=>$part){
            $ts=false;
            if(preg_match('/\b([A-Z][a-z]+\s+\d{1,2},\s+\d{4})\b/u',$part,$m)) $ts=strtotime($m[1]);
            elseif(preg_match('/\b(\d{1,2}\s+[A-Z][a-z]+\s+\d{4})\b/u',$part,$m)) $ts=strtotime($m[1]);
            elseif(preg_match('/\b(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4})\b/u',$part,$m)) $ts=strtotime(str_replace('.','-',$m[1]));
            if($ts){$date=date('Y-m-d',$ts);$dateIndex=$i;break;}
        }
        if($date==='') continue;
        $labels=[];
