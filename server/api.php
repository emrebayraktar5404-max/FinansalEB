...
{
    // Yerli + küresel kaynaklar. Haberler ekonomi ile sınırlı değildir; siyaset,
    ...
    $queries = [
        ...
        ['q' => '(site:apnews.com OR site:bbc.com/news OR site:cnbc.com) (economy OR politics OR conflict OR jobs OR energy OR technology OR health OR sports)', 'source' => 'Küresel ana kaynaklar', 'country'=>'Küresel', 'category'=>'Dünya · hayat · piyasa'],
        ['q' => '(Federal Reserve OR ECB OR Bank of England OR Bank of Japan OR IMF OR OECD OR World Bank) economy rates inflation employment', 'source' => 'Resmî küresel kurumlar', 'country'=>'Küresel', 'category'=>'Resmî veri · ekonomi'],
        ['q' => '(IEA OR EIA OR OPEC) oil gas energy supply prices', 'source' => 'Enerji kurumları', 'country'=>'Küresel', 'category'=>'Petrol · enerji'],
        ['q' => '(ILO OR Eurostat OR BLS) unemployment employment wages jobs', 'source' => 'İstihdam kurumları', 'country'=>'Küresel', 'category'=>'İstihdam · maaş'],
        ['q' => '(UNESCO OR OECD education OR World Bank education) education students skills employment', 'source' => 'Eğitim kurumları', 'country'=>'Küresel', 'category'=>'Eğitim'],
        ['q' => '(FAO OR USDA) food agriculture wheat drought prices', 'source' => 'Gıda ve tarım kurumları', 'country'=>'Küresel', 'category'=>'Gıda · tarım'],
        ['q' => '(site:aljazeera.com OR site:dw.com OR site:france24.com OR site:nikkei.com OR site:scmp.com) (politics OR war OR economy OR trade OR energy)', 'source' => 'Bölgesel dünya kaynakları', 'country'=>'Küresel', 'category'=>'Jeopolitik · ticaret'],
    ];
    // Tek bir yayıncı akışı kaplamasın: her sorgudan sınırlı sayıda kayıt alıp sıra sıra birleştiriyoruz.
    $newsBuckets = [];
    foreach ($queries as $q) {
        try { $bucket=array_slice(fetchGoogleNewsRss($q['q'], $q['source'], $config),0,4); foreach($bucket as &$item){$item['country']=$q['country']??'';$item['category']=$q['category']??'';} unset($item); $newsBuckets[]=$bucket; }
    ...
    $ipo = ['items'=>[],'news'=>[],'sources'=>getIpoSourceCatalog()];
    try { $ipo = fetchIpoBundle($config); } catch (Throwable $e) { error_log('[content/ipo] '.$e->getMessage()); }
    ...
    return [
        ...
        'ipoItems' => $ipo['items'] ?? [],
        'ipoNews' => $ipo['news'] ?? [],
        'ipoSources' => $ipo['sources'] ?? getIpoSourceCatalog(),
        'updatedAt' => date(DATE_ATOM),
...
function getIpoSourceCatalog(): array
{
    return [
        ['name'=>'SPK Bültenleri','role'=>'Halka arz onayları ve kurul kararları','url'=>'https://spk.gov.tr/spk-bultenleri'],
        ['name'=>'KAP','role'=>'İzahname, fiyat tespit/analist raporları ve sonuçlar','url'=>'https://www.kap.org.tr/tr/bildirim-sorgu'],
        ['name'=>'Borsa İstanbul','role'=>'Kota alınma ve ilk işlem günü duyuruları','url'=>'https://www.borsaistanbul.com/sirketler/halka-arz-ve-borsada-islem-gorme'],
        ['name'=>'Halkarz.com','role'=>'Kolay okunur takvim; resmî kaynaktan doğrulanmalıdır','url'=>'https://halkarz.com/'],
    ];
}

function fetchIpoBundle(array $config): array
{
    $items=[];
    try {
        $res=httpRequest('https://halkarz.com/',['connectTimeout'=>2,'timeout'=>4,'headers'=>['Accept: text/html,application/xhtml+xml']],$config);
        $text=htmlToText($res['body']);
        $lines=array_values(array_filter(array_map('trim',preg_split('/\R/u',$text)?:[]),static fn($x)=>$x!==''));
        $months='Ocak|Şubat|Mart|Nisan|Mayıs|Haziran|Temmuz|Ağustos|Eylül|Ekim|Kasım|Aralık';
        for($i=0;$i<count($lines);$i++){
            $symbol='';
            if(preg_match('/^[A-ZÇĞİÖŞÜ0-9]{3,7}$/u',$lines[$i])) $symbol=$lines[$i];
            if($symbol==='') continue;
            $name='';$dateText='';$url='https://halkarz.com/';
            for($j=$i+1;$j<min(count($lines),$i+7);$j++){
                $line=$lines[$j];
                if($name==='' && mb_strlen($line)>8 && (mb_stripos($line,'A.Ş')!==false || mb_stripos($line,'Anonim')!==false)) $name=$line;
                if($dateText==='' && (preg_match('/\b\d{1,2}(?:[-, ]+\d{1,2})*\s+('.$months.')\s+20\d{2}\b/u',$line) || mb_stripos($line,'Ertelendi')!==false)) $dateText=$line;
            }
            if($name==='' || $dateText==='') continue;
            $status=mb_stripos($dateText,'Ertelendi')!==false?'ertelendi':'takvim';
            $items[]=['symbol'=>$symbol,'name'=>$name,'demandDates'=>$dateText,'status'=>$status,'url'=>$url,'source'=>'Halkarz.com takvimi'];
            if(count($items)>=24) break;
        }
    } catch(Throwable $e) {
        error_log('[ipo/calendar] '.$e->getMessage());
    }

    $news=[];
    try {
        $news=array_slice(fetchGoogleNewsRss('(halka arz OR fiyat tespit raporu OR izahname OR işlem görmeye başlayacak) (KAP OR SPK OR Borsa İstanbul)','Halka arz gündemi',$config),0,16);
    } catch(Throwable $e) {
        error_log('[ipo/news] '.$e->getMessage());
    }

    // Resmî kaynaklara yönelik doğrudan arama bağlantıları; otomatik ayrıştırma
    // başarısız olsa bile kullanıcı raporları resmi kaynaktan açabilsin.
    foreach($news as &$n){
        $n['category']='Halka arz / şirket raporu';
        $n['country']='Türkiye';
        $n['titleTr']=$n['title']??'';
    }
    unset($n);
...
function getSourceCatalog(): array
{
    return [
        ...
        ['name'=>'Reuters','role'=>'Küresel piyasa, siyaset, enerji ve jeopolitik','url'=>'https://www.reuters.com/'],
        ['name'=>'Associated Press','role'=>'Küresel haber teyidi','url'=>'https://apnews.com/'],
        ['name'=>'BBC','role'=>'Dünya, ekonomi ve siyaset','url'=>'https://www.bbc.com/news'],
        ['name'=>'CNBC','role'=>'ABD ve küresel piyasalar','url'=>'https://www.cnbc.com/'],
        ['name'=>'Anadolu Ajansı','role'=>'Türkiye ekonomi, eğitim, spor ve dünya gündemi','url'=>'https://www.aa.com.tr/tr'],
        ['name'=>'TRT Haber','role'=>'Türkiye, ekonomi, eğitim, spor ve dünya','url'=>'https://www.trthaber.com/'],
        ['name'=>'EPDK','role'=>'Petrol, doğal gaz, LPG ve elektrik','url'=>'https://www.epdk.gov.tr/'],
        ['name'=>'IEA','role'=>'Küresel enerji ve petrol piyasası','url'=>'https://www.iea.org/'],
        ['name'=>'U.S. EIA','role'=>'Petrol, doğal gaz ve enerji verileri','url'=>'https://www.eia.gov/'],
        ['name'=>'OPEC','role'=>'Petrol arzı ve piyasa raporları','url'=>'https://www.opec.org/'],
        ['name'=>'ILO','role'=>'Küresel istihdam ve ücretler','url'=>'https://www.ilo.org/'],
        ['name'=>'Eurostat','role'=>'Avrupa ekonomi ve istihdam verileri','url'=>'https://ec.europa.eu/eurostat/'],
        ['name'=>'UNESCO','role'=>'Küresel eğitim gelişmeleri','url'=>'https://www.unesco.org/'],
        ['name'=>'FAO','role'=>'Gıda, tarım ve fiyat gelişmeleri','url'=>'https://www.fao.org/'],
        ['name'=>'WHO','role'=>'Küresel sağlık gelişmeleri','url'=>'https://www.who.int/'],
...
function translateForeignNewsTitlesTr(array $items, array $config, int $max=18): array
{
    $indexes=[];$titles=[];
    foreach($items as $i=>$item){
        $country=(string)($item['country']??'');
        if($country===''||$country==='Türkiye') { $items[$i]['titleTr']=$item['title']??''; continue; }
        if(count($titles)>=$max) break;
        $title=trim((string)($item['title']??'')); if($title==='') continue;
        $indexes[]=$i;$titles[]=$title;
    }
    if(!$titles) return $items;
    $sep="\n§§FINANSALEB§§\n";
    try{
        $url='https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=tr&dt=t&q='.rawurlencode(implode($sep,$titles));
        $res=httpRequest($url,['connectTimeout'=>2,'timeout'=>4,'headers'=>['Accept: application/json']],$config);
        $json=json_decode($res['body'],true);
        $translated=''; foreach(($json[0]??[]) as $part) if(is_array($part)) $translated.=(string)($part[0]??'');
        $chunks=preg_split('/\s*§§FINANSALEB§§\s*/u',$translated)?:[];
        if(count($chunks)===count($indexes)){
            foreach($indexes as $k=>$idx){$tr=trim((string)$chunks[$k]);$items[$idx]['titleTr']=$tr!==''?$tr:$items[$idx]['title'];$items[$idx]['translated']=$tr!=='';}
        }
    }catch(Throwable $e){ error_log('[content/translate] '.$e->getMessage()); }
    foreach($items as $i=>$item) if(!isset($items[$i]['titleTr'])) $items[$i]['titleTr']=$item['title']??'';
    return $items;
}
...
function kapRowPercent(string $html, array $labels): ?float
{
    ...
    foreach ($xpath->query('//tr') ?: [] as $tr) {
        ...
        $matched=false;
        foreach($labels as $label){ if(mb_stripos($rowText,$label)!==false){$matched=true;break;} }
        if(!$matched) continue;
        // Etiket hücresinden sonraki hücreleri tercih et. Böylece günlük oran,
        // giriş/çıkış ve performans alanları birbirine kaymaz.
        $labelIndex=0;
        foreach($cells as $i=>$cell){
            foreach($labels as $label){if(mb_stripos($cell,$label)!==false){$labelIndex=$i;break 2;}}
        }
        for($i=$labelIndex+1;$i<count($cells);$i++){
            if(preg_match('/^-?\s*%?\s*([0-9]+(?:[.,][0-9]+)?)\s*%?$/u',$cells[$i],$m)) return parseLocaleNumber($m[1]);
            if(preg_match('/(?:^|\s)%?\s*([0-9]+(?:[.,][0-9]+)?)\s*%?(?:\s|$)/u',$cells[$i],$m)) return parseLocaleNumber($m[1]);
        }
    }
    return null;
}
...
function fetchKapFundFees(string $code, string $name, array $config): array
{
    ...
    // KAP tablosunu satır bazında okuyoruz. v0.3.9'daki düz metin regex'i
    // komşu satırların sayılarını giriş/çıkış/performance alanlarına taşıyabiliyordu.
    $daily=kapRowPercent($html,['Yönetim Ücreti Oranı (Günlük)','Yönetim Ücreti Oranı Günlük']);
    $annual=kapRowPercent($html,['Yönetim Ücreti Oranı (Yıllık)','Yönetim Ücreti Oranı Yıllık']);
    $entry=kapRowPercent($html,['Giriş Komisyonu']);
    $exit=kapRowPercent($html,['Çıkış Komisyonu']);
    $performance=kapRowPercent($html,['Performans Ücreti Oranı','Performans Ücreti']);
    $expense=kapRowPercent($html,['Fon Toplam Gider Oranı']);

    // Sadece yönetim ücretleri için kontrollü metin yedeği kullanılır.
    // Komisyonlarda yedek regex kullanılmaz; yanlış değer göstermektense null daha güvenlidir.
    ...
    if($annual===null && preg_match('/Yönetim Ücreti Oranı \(Yıllık\)[^0-9]{0,120}([0-9]+(?:[.,][0-9]+)?)/ui',$text,$m)) $annual=parseLocaleNumber($m[1]);
    if($daily===null && preg_match('/Yönetim Ücreti Oranı \(Günlük\)[^0-9]{0,120}([0-9]+(?:[.,][0-9]+)?)/ui',$text,$m)) $daily=parseLocaleNumber($m[1]);

    $found=($annual!==null||$daily!==null||$entry!==null||$exit!==null||$performance!==null||$expense!==null);
    return [
        ...
        'note'=>$found?'KAP resmi fon sayfasındaki ücret satırlarından otomatik okundu.':'KAP sayfası bulundu; ücret bilgisi doğrulanamadı. Yanlış değer yazılmadı; gerekirse manuel giriş kullanılabilir.'
    ];
}
