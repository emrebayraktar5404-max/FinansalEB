...
function fetchGoogleNewsRss(string $query, string $source, array $config): array
{
    ...
    foreach ($items[1] ?? [] as $item) {
        $title = xmlTag($item,'title'); $link=xmlTag($item,'link'); $pub=xmlTag($item,'pubDate'); $desc=xmlTag($item,'description');
        if ($title==='') continue;
        $out[]=['title'=>html_entity_decode(strip_tags($title),ENT_QUOTES|ENT_HTML5,'UTF-8'),'url'=>html_entity_decode($link,ENT_QUOTES|ENT_HTML5,'UTF-8'),'publishedAt'=>date(DATE_ATOM, strtotime($pub) ?: time()),'summary'=>trim(html_entity_decode(strip_tags($desc),ENT_QUOTES|ENT_HTML5,'UTF-8')),'source'=>$source];
    }
    return $out;
}
function xmlTag(string $xml,string $tag): string { if (preg_match('/<'.preg_quote($tag,'/').'(?:\s[^>]*)?>(.*?)<\/'.preg_quote($tag,'/').'>/is',$xml,$m)) return trim($m[1]); return ''; }
...
function fetchInvestorPortfolios(array $config): array
{
    ...
    foreach($managers as $m){
        try { $row=fetchLatest13F($m['name'],$m['cik'],$config); if($row) $out[]=$row; } catch(Throwable $e){ error_log('[13F '.$m['name'].'] '.$e->getMessage()); }
    }
    return $out;
}
...
function fetchLatest13F(string $manager,string $cik,array $config): ?array
{
    ...
    preg_match_all('/<infoTable>(.*?)<\/infoTable>/is',$x,$rows); $hold=[];
