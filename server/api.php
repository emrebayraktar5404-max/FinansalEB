...
try {
    switch ($action) {
        ...
        case 'content':
            $data = cached('content_v5', 900, fn() => fetchContentBundle($config));
            respond(['ok' => true, 'data' => $data]);
            break;
...
function fetchContentBundle(array $config): array
{
    ...
    $macro = array_values(array_filter($macro, static function($e) use ($today,$limitDate){ $d=(string)($e['date']??''); return $d!=='' && $d >= $today && $d <= $limitDate; }));
